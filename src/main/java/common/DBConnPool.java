...
public class DBConnPool {
	...
	public ResultSet rs;
	/*커넥션 풀 설정을 위해 server.xml, context,xml파일에 엘리먼트를
	추가해야한다. 자세한 내용은 교재 194페이지 참조
	*/
	public DBConnPool() {
		try {
			...
			디렉토리를 얻어온다. 여기서 얻어오는 톰캣의 루트 디렉토리
			명은 java:comp/env로 이미 정해져 있으므로 그대로 사용하면
			된다.
			*/
			...
			// 커넥션풀에 생성해 놓은 커넥션 객체를 가져다가 사용한다.
	...
	/*
	사용이 끝난 객체는 Pool에 만납한다. 즉 여기서의 close()는
	객체의 소멸이 아닌 반납이다. 
	 */
	public void close() {
		try {
